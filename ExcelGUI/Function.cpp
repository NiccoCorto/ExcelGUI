...
void Function::update(Subject* subject) {
    ...
    case MEAN:
        ...
        break;
    case MIN:
        value = (*std::min_element(cells.begin(), cells.end(), [](const std::shared_ptr<Cell>& a, const std::shared_ptr<Cell>& b) {
            return a->getValue() < b->getValue();  // Dereference shared_ptr to access Cell's value
            }))->getValue();
        break;
    case MAX:
        value = (*std::max_element(cells.begin(), cells.end(), [](const std::shared_ptr<Cell>& a, const std::shared_ptr<Cell>& b) {
            return a->getValue() < b->getValue();  // Dereference shared_ptr to access Cell's value
            }))->getValue();
        break;
    }
